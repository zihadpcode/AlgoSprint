...
- [ ] Run visible tests works in the browser for guests and members and saves nothing; with the provider disabled, Submit fails clearly. With `CODE_RUNNER_PROVIDER=sandbox`, a correct Submit records a verified solve and a wrong one reports the summary only; submitted code runs in the in-process QuickJS sandbox, the user-approved exception described in PHASE-8-GUIDE.md. With `CODE_RUNNER_PROVIDER=judge0`, enable it only after verifying the independently operated provider, authentication and JavaScript language ID. Then verify hidden-case submission behavior using the Phase 8–9 checklists.
...
| Auth, provider and browser checks | 2026-09-21: user verified registration, email confirmation, login and logout with two accounts on the production origin. 2026-09-24: code runner enabled with `CODE_RUNNER_ENABLED=true` and `CODE_RUNNER_PROVIDER=sandbox` (Production), redeployed on `17a9c25` (PR #27). Signed-in Submit on `bake-batches`: the unchanged starter code returned `RUNTIME_ERROR` 0/8 and a correct solution returned `ACCEPTED` 8/8 (17 ms slowest case, 95 KB peak), both showing only the summary. Verified via Supabase SQL: both attempts stored with empty public `cases`, and `UserProgress` shows `SOLVED` with `verifiedRevision` 1 and `selfMarked` false. 2026-09-24 (later): the user reported notes (save, reload, unsaved edit discarded on reload, delete), bookmark and review flags, manual solve and its undo, dashboard, progress, review and roadmap pages all working on production. Supabase SQL afterwards: no note rows or blank placeholders remained, flags were cleared, a manually solved problem was back to `NOT_STARTED`, and the verified `bake-batches` solve kept `verifiedRevision` 1 through flag changes. Undoing verified solves and attempts was requested and added afterwards (PHASE-9-GUIDE.md, September 24 follow-up). Remaining checklist items above are still pending. |
| Advisors and logs | 2026-09-24 (Supabase advisors and 24 hours of logs after the live checks): **ERROR** `rls_disabled_in_public` on `public._prisma_migrations`: Supabase's default privileges had granted `anon` and `authenticated` every privilege on Prisma's migration history, reachable through the Data API with the publishable key. Fixed by migration `202609240001_lock_migration_history` (enable RLS, revoke both roles; guarded so plain PostgreSQL is unaffected), Applied to production on 2026-09-24 through the Supabase SQL Editor (the migration file verbatim plus the `_prisma_migrations` row Prisma would write, checksum `91282fec…ccf0`, identical to Prisma's own locally), because this developer network drops TLS to the Supabase pooler while plain Postgres elsewhere works; `migrate deploy` from another network will see it as applied. Re-run: RLS on, `anon`/`authenticated` hold no privileges, and the advisor ERROR is gone. **WARN** leaked-password protection is disabled; it requires the Supabase Pro plan, so it stays off on Free. **INFO** `rls_enabled_no_policy` on all 22 `app` tables is intentional (the schema is not exposed and has no browser-role grants; Prisma authorizes on the server), and 11 unused indexes are expected at this traffic level; keep them. Logs: no Postgres errors, no API gateway 4xx/5xx, one invalid-credentials password sign-in, and routine PostgREST idle-timeout messages. Vercel runtime logs were not inspected from this environment. |
