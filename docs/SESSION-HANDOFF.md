...
On 2026-09-23 a build-trace check found that the Vercel function for the problem page would not include the QuickJS packages (only the worker's source string requires them), so Submit would have failed as `INTERNAL_ERROR` once the sandbox was enabled. `outputFileTracingIncludes` now ships them with that page and `npm run test:trace` guards it in CI. On 2026-09-24, after PR #27 merged, the user added `CODE_RUNNER_PROVIDER=sandbox` and set `CODE_RUNNER_ENABLED=true` in Vercel Production and redeployed. Signed-in Submit now works live: a correct solution recorded a verified solve, and a failing one reported the summary only (see the DEPLOYMENT.md release record). On 2026-09-24 the user verified notes, bookmarks, review and manual progress live, and asked to be able to undo a verified solve and an attempt; see the September 24 follow-up in PHASE-9-GUIDE.md. Next: the remaining live checklist items in DEPLOYMENT.md (two-user privacy, interview flow, screenshots, runtime logs and advisors).
