...
This is the original user-provided brief, preserved for continuity. Phases 1–12 are merged. Phase 13 protected content administration is implemented and documented in PR #14: manager/filtering, structured create/edit fields, archive/safe deletion, atomic create-only JSON imports and existing-roadmap placement. Current database role checks, revision conflicts and transactions preserve user history; imported code remains inert and new custom runner contracts are not added. All 145 unit/component/migration and 61 PostgreSQL integration tests pass; full implementation CI 35408522853 passed all steps, including migrations, seeding, lint/types/build and both production HTTP suites. PR #14 records full/final-head CI and merge evidence. PHASE-13-GUIDE.md includes all 21 complete source/test/script files and the handoff is current. Pause before Phase 14 after final checks and merge. No schema/dependency changes, production data writes or deployment; live account/provider/browser checks remain pending. Earlier pauses and the original Phase 1-only instruction below are historical.
